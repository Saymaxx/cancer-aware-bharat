...
import { useDonationsMonthly, usePatientIntakeMonthly } from '../../api/hooks';
...
export default function AnalyticsTab({ apiToken }: { apiToken: string | null }) {
  ...
  return (
    <div className="space-y-6 animate-[fadeInUp_0.4s_ease-out]">
      ...
      {/* Honest empty state -- no real hours-tracking data exists yet
          (see backend/app/routers/analytics.py), so this chart is dropped
          rather than showing fabricated numbers. */}
      <div className="bg-white rounded-2xl border border-slate-200/60 p-6 shadow-xs">
        <h3 className="text-sm font-bold text-slate-900 mb-2">Monthly Volunteer Hours</h3>
        <p className="text-xs text-slate-400">Volunteer hours tracking isn't built yet -- this chart will appear once that data exists.</p>
      </div>
    </div>
  );
}
