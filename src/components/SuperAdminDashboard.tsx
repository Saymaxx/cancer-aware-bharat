...
import {
  ...
  PieChart, LayoutDashboard, Megaphone, FolderArchive, Menu
} from 'lucide-react';
...
export default function SuperAdminDashboard({ onPageChange, onLogout }: { onPageChange: (page: string) => void; onLogout: () => void }) {
  ...
  const [sentNotifications, setSentNotifications] = useState<SentNotification[]>(INITIAL_SENT_NOTIFICATIONS);
  ...
  // Profile state
  const [profileName, setProfileName] = useState('Board Administrator');
  ...
  const filteredHospitals = useMemo(() => {
    return hospitals.filter(h => {
      ...
      const isRecommendedOrProcessed = h.status === 'Recommended by Admin' || h.status === 'Recommended to Super Admin' || h.status === 'Approved' || h.status === 'Rejected' || h.status === 'Info Requested';
    ...
    <div className="min-h-screen bg-slate-50 flex text-slate-800">
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-[#f8f7ff]">
        ...
        <div className="p-4 sm:p-6 overflow-y-auto max-w-[1400px] w-full mx-auto space-y-6">
          ...
          {/* ===== TAB: PATIENTS (Read-Only Overview) ===== */}
          {activeTab === 'patients' && (
            <div className="space-y-4 animate-[fadeInUp_0.4s_ease-out]">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  { label: 'Total Patients', value: '1,420', color: 'text-rose-700 bg-rose-50 border-rose-200' },
                  { label: 'Under Treatment', value: '385', color: 'text-amber-700 bg-amber-50 border-amber-200' },
                  { label: 'Recovered', value: '892', color: 'text-emerald-700 bg-emerald-50 border-emerald-200' },
                  { label: 'Financial Aid Pending', value: '12', color: 'text-blue-700 bg-blue-50 border-blue-200' },
                ].map((s, i) => (
              ...
              <div className="bg-white rounded-2xl border border-slate-200/60 p-6 shadow-xs text-center text-slate-500">
                <Heart className="w-12 h-12 mx-auto mb-3 text-slate-300" />
                <p className="text-sm font-semibold">Patient data is managed by Regional Admins.</p>
                <p className="text-xs mt-1">Navigate to Admin Management to review admin activities on patient records.</p>
              </div>
            </div>
          )}

          {/* ===== TAB: VOLUNTEERS (Read-Only Overview) ===== */}
          {activeTab === 'volunteers' && (
            <div className="space-y-4 animate-[fadeInUp_0.4s_ease-out]">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  { label: 'Total Volunteers', value: '2,400', color: 'text-emerald-700 bg-emerald-50 border-emerald-200' },
                  { label: 'Active This Month', value: '312', color: 'text-blue-700 bg-blue-50 border-blue-200' },
                  { label: 'Pending Approval', value: '18', color: 'text-amber-700 bg-amber-50 border-amber-200' },
                  { label: 'Total Hours Logged', value: '4,460', color: 'text-purple-700 bg-purple-50 border-purple-200' },
                ].map((s, i) => (
                  <div key={i} className={`${s.color} rounded-2xl border p-4 text-center`}>
                    <p className="text-2xl font-black">{s.value}</p>
                    <p className="text-[10px] font-bold mt-1">{s.label}</p>
              ...
              <div className="bg-white rounded-2xl border border-slate-200/60 p-6 shadow-xs text-center text-slate-500">
                ...
                <p className="text-sm font-semibold">Volunteer management is handled by Regional Admins.</p>
                <p className="text-xs mt-1">Approvals, attendance tracking, and campaign assignments are delegated to regional nodes.</p>
              </div>
            </div>
          )}
          ...
          {activeTab === 'campaigns' && (
            <div className="space-y-4 animate-[fadeInUp_0.4s_ease-out]">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {[
                  { label: 'Active Campaigns', value: '6', color: 'text-indigo-700 bg-indigo-50 border-indigo-200' },
                  { label: 'Total This Year', value: '18', color: 'text-purple-700 bg-purple-50 border-purple-200' },
              ...
              <div className="bg-white rounded-2xl border border-slate-200/60 p-6 shadow-xs text-center text-slate-500">
                ...
                <p className="text-sm font-semibold">Campaign scheduling is managed by Regional Admins and Campaign Managers.</p>
                <p className="text-xs mt-1">Review campaign impact data in the Analytics or Reports tab.</p>
              </div>
            </div>
          )}
          ...
          {activeTab === 'donations' && (
            <div className="space-y-4 animate-[fadeInUp_0.4s_ease-out]">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  { label: 'Total Donations', value: '₹8,40,500', color: 'text-emerald-700 bg-emerald-50 border-emerald-200' },
                  { label: 'Individual Donors', value: '142', color: 'text-blue-700 bg-blue-50 border-blue-200' },
                  { label: 'Corporate Donors', value: '8', color: 'text-purple-700 bg-purple-50 border-purple-200' },
                  { label: 'Receipts Pending', value: '3', color: 'text-amber-700 bg-amber-50 border-amber-200' },
                ].map((s, i) => (
                  <div key={i} className={`${s.color} rounded-2xl border p-4 text-center`}>
                    <p className="text-2xl font-black">{s.value}</p>
                    <p className="text-[10px] font-bold mt-1">{s.label}</p>
              ...
              <div className="bg-white rounded-2xl border border-slate-200/60 p-6 shadow-xs text-center text-slate-500">
                ...
                <p className="text-sm font-semibold">Donation records are managed in the Admin Dashboard.</p>
                <p className="text-xs mt-1">Use Reports & Export to generate detailed financial reports.</p>
              </div>
            </div>
          )}
          ...
          {activeTab === 'blogs' && (
            <div className="bg-white rounded-2xl border border-slate-200/60 p-6 shadow-xs text-center text-slate-500 animate-[fadeInUp_0.4s_ease-out]">
              ...
              <p className="text-sm font-semibold">Blog & Content management is handled by Admins.</p>
              <p className="text-xs mt-1">Content moderation can be configured in System Settings.</p>
            </div>
          )}
          ...
          {activeTab === 'database' && (
            <div className="space-y-6 animate-[fadeInUp_0.4s_ease-out]">
              ...
              {/* Actions */}
              <div className="flex gap-3">
                <button onClick={() => showToast('Database backup initiated. Estimated time: 4 minutes.')} className="px-5 py-2.5 bg-indigo-600 text-white rounded-xl text-xs font-bold hover:bg-indigo-700 cursor-pointer flex items-center gap-1.5 shadow-sm"><FolderArchive className="w-4 h-4" /> Create Backup Now</button>
                <button onClick={() => showToast('Restore initiated from latest backup.')} className="px-5 py-2.5 bg-white text-indigo-600 border border-indigo-200 rounded-xl text-xs font-bold hover:bg-indigo-50 cursor-pointer flex items-center gap-1.5"><RefreshCw className="w-4 h-4" /> Restore Latest</button>
              </div>
            ...
            <div className="max-w-2xl animate-[fadeInUp_0.4s_ease-out]">
              <div className="bg-white rounded-2xl border border-slate-200/60 p-6 shadow-xs">
                ...
                <div className="space-y-4 text-xs">
                  ...
                  <div className="flex gap-3 pt-2">
                    <button onClick={() => showToast('Profile updated successfully!')} className="px-5 py-2.5 bg-indigo-600 text-white rounded-xl text-xs font-bold hover:bg-indigo-700 cursor-pointer shadow-sm">Save Changes</button>
